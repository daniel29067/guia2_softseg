...
class Modelo
{
   ...
   public function crearStudent($NAME, $LAST,$PROGRAM,$USERID)
{
    require("../modelo/connect_db.php");

    // Verificar si el nombre y el apellido ya están registrados
    $stmt = mysqli_prepare($mysqli, "SELECT COUNT(*) FROM students WHERE NAME = ? AND LAST = ?");
    mysqli_stmt_bind_param($stmt, "ss", $NAME, $LAST);
    ...
    if ($count > 0) {
        ...
        $stmt = mysqli_prepare($mysqli, "INSERT INTO students (NAME, LAST, PROGRAM) VALUES (?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "sss", $NAME, $LAST, $PROGRAM);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);

        // Obtener el ID autoincrementable insertado
        $student_id = mysqli_insert_id($mysqli);

        // Insertar el registro de auditoría
        $stmt_audit = mysqli_prepare($mysqli, "INSERT INTO AUDIT_STUDENTS (NEW_NAME, NEW_PROGRAM, USERID, MODIFY, PROCESS, ID_STUDENT) VALUES (?, ?, ?, NOW(), 'Insertar', ?)");
        mysqli_stmt_bind_param($stmt_audit, "ssii", $NAME, $PROGRAM, $USERID, $student_id);
